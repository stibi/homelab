...
process. The real renewable token stays inside the Agent process: it is not
written to disk or exposed to the workstation user. The role instead writes a
non-secret placeholder to `~/.vault-token` with mode `0600`. Vault-aware tools
therefore find a valid private token-helper file, while the proxy's
`use_auto_auth_token = "force"` setting discards that placeholder and forwards
the request with its AppRole token. This is required for clients such as the
Terraform Vault provider, which reject group-readable token files.
